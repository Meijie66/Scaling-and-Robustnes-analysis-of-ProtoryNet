...
train_dataset = tf.data.Dataset.from_tensor_slices((dict(x_train_encoding), train_labels)).batch(32)
test_dataset = tf.data.Dataset.from_tensor_slices((dict(x_test_encoding), test_labels)).batch(32)

# Load BERT model with 3 output classes
model = TFBertForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=200)
...
plt.savefig('accuracy_plot_200.png')
plt.show()