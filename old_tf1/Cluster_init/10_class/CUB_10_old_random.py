...
import sys
from protorynet_10_old_random import ProtoryNet
import pandas as pd
import re
...
for k in unique_labels:
    ...
    plt.scatter(cluster_points[:, 0], cluster_points[:, 1],
                s=30, color=colors(k), label=f'Cluster {k}', alpha=0.8, zorder=1)
    ...
    plt.scatter(medoid_point[0], medoid_point[1],
                marker='*', s=550, c='yellow', edgecolor='black', linewidth=1, zorder=2)
...
plt.tight_layout()
plt.savefig("random1.png", dpi=600, bbox_inches='tight')
plt.show()
'''
...
# Plot medoids as red stars
medoids_pca = pca.transform(X[k_cents])
plt.scatter(medoids_pca[:, 0], medoids_pca[:, 1], color='red', marker='*', s=200, label='Medoids')
